...
def chordStatsBySeg(data, doctorid):
    ...
    tagListDf.insert(0, "doctorid", doctorid)
...
def dataPrepare(webRaw):
    ...
    wechatColList = ["doctorid", "content_title", "start_date"]
    wechatFilterd = webRaw[wechatColList]
    
    # Remove the invalid data: content title is 0
    validWebLog = wechatFilterd[~((wechatFilterd.content_title=="") | (wechatFilterd.content_title.isnull()) | (wechatFilterd.doctorid.isnull()) | (wechatFilterd.doctorid==''))].reset_index(drop=True)
