...
# @Last Modified time: 2018-10-11 15:57:08
...
def dataPrepare(wechatRaw, webRaw):
    ...
    wechatFilterd = wechat[~wechat.doctorid.isnull()][wechatColList]
    
    # filter web data having doctorid
    webColList = ["doctorid", "content_id", "content_title", "start_date", "end_date"]
    webFilterd = web[~web.doctorid.isnull()][webColList]
...
def main():
    tag = nn.read_table()
    similar = nn.read_table()
    mapping =  mappingCbind(similar,tag)
    createDictStop()
    wechat = nn.read_table()
    web = nn.read_table()
