...
if [ "$HCPCAPA_OPENID_HOME" == "" ];then
   echo "[ERROR]Please setup HCPCAPA_OPENID_HOME first before launch algorithm"
   exit
fi
echo "[INFO]HOME directory check"
echo "[INFO]HOME directory is ""$HCPCAPA_OPENID_HOME"

#PYTHON LIB HOME check
if [ "$PYTHONPATH" == "" ];then
   export PYTHONPATH=$HCPCAPA_OPENID_HOME/lib
   export PYTHONPATH=$PATHONPATH:$HCPCAPA_OPENID_HOME/.source_file
else
   export PYTHONPATH=$PYTHONPATH:$HCPCAPA_OPENID_HOME/lib
   export PYTHONPATH=$PATHONPATH:$HCPCAPA_OPENID_HOME/.source_file
fi
...
#configuration check
if [ ! -f "$HCPCAPA_OPENID_HOME/etc/config.json" ];then
...
export COMM="$PYTHONBINARY $HCPCAPA_OPENID_HOME/lib/launch.py "

echo "[INFO]HCPCAPA is running"
eval $COMM
